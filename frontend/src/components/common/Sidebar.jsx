...
const landlordLinks = [
  { name: 'Home', icon: Home, path: '/landlord/home' },
  { name: 'Properties', icon: Building2, path: '/landlord/properties' },
  { name: 'Requests', icon: FileText, path: '/landlord/requests' },
...
const tenantLinks = [
  { name: 'Home', icon: Home, path: '/tenant/home' },
  { name: 'Requests', icon: FileText, path: '/tenant/requests' },
    ...
    <>
      ...
      >
        <div className="hidden lg:flex h-20 items-center justify-between px-6 border-b border-white/10">
          {!collapsed && <img src="/logo.png" alt="Urugo" className="h-8 w-auto brightness-0 invert" />}
          <button
          ...
          <div className={`flex items-center gap-3 p-2 rounded-xl mb-2 ${collapsed && 'lg:justify-center'}`}>
            ...
            {(!collapsed || mobileOpen) && (
              <div className="min-w-0">
                ...
                <p className="text-[10px] font-black text-white/60 uppercase tracking-widest">{user?.role}</p>
              </div>
            )}
          </div>
