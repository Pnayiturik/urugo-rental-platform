        ...
        <div className="lg:col-span-7 space-y-8">
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
            ...
            <div className="p-6 bg-slate-50 rounded-[2rem] border border-slate-100">
              ...
              <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">rent fees</p>
          ...
          {/* Property Details: Furnishing, Amenities, Rooms, Location */}
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mt-4">
            <div className="bg-slate-50 rounded-2xl border border-slate-100 p-5">
              <h3 className="text-sm font-black uppercase tracking-widest text-slate-500 mb-2">Furnishing</h3>
              <p className="font-bold text-slate-900">{property.furnishingStatus ? property.furnishingStatus.replace('-', ' ') : 'N/A'}</p>
            </div>
            <div className="bg-slate-50 rounded-2xl border border-slate-100 p-5">
              <h3 className="text-sm font-black uppercase tracking-widest text-slate-500 mb-2">Amenities</h3>
              <p className="font-bold text-slate-900">{Array.isArray(property.utilitiesIncluded) && property.utilitiesIncluded.length ? property.utilitiesIncluded.join(', ') : 'N/A'}</p>
            </div>
            <div className="bg-slate-50 rounded-2xl border border-slate-100 p-5">
              <h3 className="text-sm font-black uppercase tracking-widest text-slate-500 mb-2">Rooms</h3>
              <p className="font-bold text-slate-900">
                {units.length > 0
                  ? `${units.reduce((sum, u) => sum + (u.bedrooms || 0), 0)} Bedrooms, ${units.reduce((sum, u) => sum + (u.bathrooms || 0), 0)} Bathrooms`
                  : 'N/A'}
              </p>
            </div>
            <div className="bg-slate-50 rounded-2xl border border-slate-100 p-5">
              <h3 className="text-sm font-black uppercase tracking-widest text-slate-500 mb-2">Location</h3>
              <p className="font-bold text-slate-900">{formatAddress(property.address)}</p>
              {property.locationDetails?.coordinates?.lat && property.locationDetails?.coordinates?.lng && (
                <a
                  href={`https://www.google.com/maps/search/?api=1&query=${property.locationDetails.coordinates.lat},${property.locationDetails.coordinates.lng}`}
                  target="_blank"
              ...
              {property.locationDetails?.landmarksText && (
                <div className="text-xs text-slate-500 mt-1">Landmarks: {property.locationDetails.landmarksText}</div>
              )}
            </div>
          </div>
